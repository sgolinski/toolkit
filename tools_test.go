...
func TestTools_UploadFiles(t *testing.T) {
	...
	for _, e := range uploadTests {
		...
		var testTools Tools
		testTools.AllowedFilesTypes = e.allowedTypes
...
var jsonTests = []struct {
	...
	{name: "two json files", json: `{"foo":"1"}{"alfa":"beta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo":1"`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "unknown field", json: `{"fooooo":1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown field", json: `{"fooooo":"1"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		testTool.AllowedUnknownFields = e.allowUnknown
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status code reuturned; expected 503 but got %d", rr.Code)
	}
